...
// ─── Lógica de carga fuera del componente — el React Compiler no la toca ────
async function loadMapData(
  ...
  const { setLoading, setLoadError, setIslands, setLevels,
          setLocalLives, setLocalLivesResetAt } = callbacks;
...
export default function MapaPage() {
  ...
  // ─── Efecto principal ─────────────────────────────────────────────────────
  // Guard por userId: cada userId distinto dispara exactamente una carga.
  // Si authLoading es true, no hacemos nada. Si user es null, redirigimos.
  // profile puede llegar después — esperamos solo cuando userId ya existe.
  ...
  useEffect(() => {
    ...
    return () => {
      controller.abort();
      // Resetear el guard en el cleanup — necesario para React Strict Mode
      // (que desmonta/remonta efectos en dev) y para cuando el usuario vuelve
      // al mapa después de jugar (window.location.href desmonta el componente).
  ...
  function handleRetry() {
    ...
    loadedForRef.current = null; // resetear guard para permitir retry
    setLoadError(false);
    const controller = new AbortController();
    loadMapData(user.id, profile.stage, controller.signal, {
      setLoading, setLoadError, setIslands, setLevels,
  ...
  // Mostrar loading si auth carga, el mapa carga, o user existe pero profile aún no llegó
  // El último caso ocurre justo después del redirect OAuth
        ...
        <header className="sticky top-0 z-50 w-full px-4 py-2.5 bg-[rgba(10,10,15,0.97)] border-b border-white/[0.07] flex items-center justify-between gap-3">
          <div className="flex items-center gap-2.5">
            <button
              ...
              onClick={() => setShowAvatars((v) => !v)} title="Cambiar avatar">
              {avatarEmoji}
            </button>
            <div>
                    ...
                    return (
                      <div key={level.id} className="absolute -translate-x-1/2 -translate-y-1/2 flex flex-col items-center z-2"
                        ...
                        <div className="flex gap-px mb-1">
                          {[1, 2, 3].map((s) => (
                            <span key={s} style={{ fontSize: "0.65rem", opacity: isDone && level.stars >= s ? 1 : 0.18, filter: isDone && level.stars >= s ? `drop-shadow(0 0 4px ${theme.node})` : "none" }}>⭐</span>
                          ))}
                        </div>
                        <button
                          ...
                          {isLocked ? "🔒" : level.icon}
                          {isCurrent && <span className="absolute -inset-2 rounded-full border-2 border-transparent animate-[pingAnim_2s_ease-out_infinite] opacity-0" style={{ borderColor: theme.node }} />}
                        </button>
