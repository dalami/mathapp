...
export default function MapaPage() {
  ...
  // ─── Vidas al volver del background (PWA) ──────────────────────────────────
  useEffect(() => {
    if (!user) return;
    const uid = user.id;
                  ...
                  {islandLevels.map((level, i) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <button
                          className={`rounded-full border-[3px] border-transparent bg-[#2a2a35] text-white flex items-center justify-center relative outline-none transition-transform duration-140ms active:scale-[0.91] ${
                            level.is_boss
                              ? "w-18 h-18 text-[1.7rem]"
                              : "w-16 h-16 text-[1.4rem]"
                          } ${isCurrent ? "animate-[pulseNode_2s_ease-in-out_infinite]" : ""} ${
                            isLocked
                              ? "cursor-not-allowed opacity-50 grayscale-[0.6]"
                              : "cursor-pointer"
                          }`}
                          style={{
                            background: isDone
