...
export default function MapaPage() {
  ...
  // profileRef para acceder al profile actualizado dentro de closures
  const profileRef = useRef(profile);
  useEffect(() => {
    profileRef.current = profile;
  }, [profile]);

  // callbacks ref para acceder a los setters dentro de closures
  ...
  useEffect(() => {
    ...
    const handleVisibilityChange = async () => {
      ...
      if (currentProfile?.stage) {
        // Recarga datos en background sin resetear loadedForRef
        // Así el efecto principal no vuelve a correr
        const controller = new AbortController();
        loadMapData(
          uid,
  ...
  if (authLoading || loading || !user || !profile)
    return <LoadingScreen mountedAt={mountedAt} />;
