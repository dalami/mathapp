...
// Paleta stage 4 — universidad — 12 islas, colores intensos y distintos
const ISLAND_THEMES: Record<
  number,
  { bg: string; node: string; nodeBorder: string; shadow: string; label: string }
> = {
  1:  { bg: "#0e1f1a", node: "#10B981", nodeBorder: "#047857", shadow: "#10B98155", label: "#6EE7B7" }, // Esmeralda
  2:  { bg: "#1a0e1a", node: "#C084FC", nodeBorder: "#7E22CE", shadow: "#C084FC55", label: "#E9D5FF" }, // Violeta intenso
  3:  { bg: "#1a120e", node: "#F59E0B", nodeBorder: "#92400E", shadow: "#F59E0B55", label: "#FDE68A" }, // Ámbar
  4:  { bg: "#0e1520", node: "#38BDF8", nodeBorder: "#0369A1", shadow: "#38BDF855", label: "#BAE6FD" }, // Celeste
  5:  { bg: "#200e0e", node: "#F43F5E", nodeBorder: "#9F1239", shadow: "#F43F5E55", label: "#FECDD3" }, // Rosa rojo
  6:  { bg: "#0e1e1e", node: "#06B6D4", nodeBorder: "#155E75", shadow: "#06B6D455", label: "#A5F3FC" }, // Cian
  7:  { bg: "#1e200e", node: "#84CC16", nodeBorder: "#3F6212", shadow: "#84CC1655", label: "#D9F99D" }, // Lima brillante
  8:  { bg: "#200e16", node: "#EC4899", nodeBorder: "#9D174D", shadow: "#EC489955", label: "#FBCFE8" }, // Fucsia
  9:  { bg: "#0e0e20", node: "#818CF8", nodeBorder: "#3730A3", shadow: "#818CF855", label: "#C7D2FE" }, // Índigo
  10: { bg: "#201a0e", node: "#FB923C", nodeBorder: "#C2410C", shadow: "#FB923C55", label: "#FED7AA" }, // Naranja
  11: { bg: "#0e1a14", node: "#34D399", nodeBorder: "#065F46", shadow: "#34D39955", label: "#A7F3D0" }, // Verde menta
  12: { bg: "#1a0e0e", node: "#FCA5A5", nodeBorder: "#B91C1C", shadow: "#FCA5A555", label: "#FEE2E2" }, // Rojo suave
};
...
async function loadMapData(
  ...
  stage: number,
  signal: AbortSignal,
  callbacks: LoadMapCallbacks,
) {
  const { setLoading, setLoadError, setIslands, setLevels, setLocalLives, setLocalLivesResetAt, setLocalCoins } = callbacks;
  setLoading(true);
  setLoadError(false);
  try {
    void (async () => {
    ...
    const islandsRes = await supabase
      .from("islands")
    ...
    const [levelsRes, progressRes] = await Promise.all([
      supabase
        .from("levels")
        .select("id,name,icon,order_index,island_id,is_boss,time_limit_secs,questions_count,unlock_requires")
    ...
    void (async () => {
      try {
        ...
        if (!signal.aborted && data?.coins != null) setLocalCoins(data.coins);
      } catch { /* silencioso */ }
    })();
  } catch (e) {
    if (signal.aborted) return;
...
export default function Mapa4Page() {
  const router = useRouter();
  const { profile, user, loading: authLoading, signOut, profileError } = useAuth();
  ...
  const callbacksRef = useRef<LoadMapCallbacks>({
    setLoading, setLoadError, setIslands, setLevels,
    setLocalLives, setLocalLivesResetAt, setLocalCoins,
  });
  useEffect(() => { // eslint-disable-line react-hooks/exhaustive-deps
    callbacksRef.current = { setLoading, setLoadError, setIslands, setLevels, setLocalLives, setLocalLivesResetAt, setLocalCoins };
  });
  ...
  useEffect(() => {
    ...
    loadMapData(user.id, profile.stage, controller.signal, callbacksRef.current);
  ...
  useEffect(() => {
    ...
    const handleVisibilityChange = async () => {
      ...
      if (currentProfile?.stage) {
        const controller = new AbortController();
        loadMapData(uid, currentProfile.stage, controller.signal, callbacksRef.current);
      }
    };
  ...
  if (!authLoading && user && profileError && !profile) {
    return (
      <div className="min-h-dvh bg-[#0a0a0f] flex flex-col items-center justify-center gap-5 font-[Nunito,sans-serif] px-6 text-center">
        ...
        <div className="text-white/60 text-sm font-bold">No se pudo cargar el perfil</div>
        <button className="px-6 py-3 rounded-2xl bg-white/10 border border-white/15 text-white font-extrabold text-sm cursor-pointer hover:bg-white/18 transition-colors" onClick={() => window.location.reload()}>Reintentar 🔄</button>
      </div>
    );
  }
  ...
  if (loadError) {
    return (
      <div className="min-h-dvh bg-[#0a0a0f] flex flex-col items-center justify-center gap-5 font-[Nunito,sans-serif] px-6 text-center">
        ...
        <button className="px-6 py-3 rounded-2xl bg-white/10 border border-white/15 text-white font-extrabold text-sm cursor-pointer hover:bg-white/18 transition-colors" onClick={handleRetry}>Reintentar 🔄</button>
      </div>
    );
  }
  ...
  return (
    <>
      <div className="fixed inset-0 pointer-events-none z-0 overflow-hidden bg-[#0a0a0f]" aria-hidden="true">
        {STARS_BG.map((s, i) => (
          <span key={i} className="absolute rounded-full bg-white/75 animate-[twinkle_ease-in-out_infinite_alternate]"
      ...
      <div className="relative z-10 min-h-dvh font-[Nunito,sans-serif] text-white flex flex-col items-center max-w-130 mx-auto" ref={topRef}>
        ...
        <header className="sticky top-0 z-50 w-full px-4 py-2.5 bg-[rgba(10,10,15,0.97)] border-b border-white/[0.07] flex items-center justify-between gap-3">
          <div className="flex items-center gap-2.5">
            <button className="w-10 h-10 rounded-full shrink-0 bg-linear-to-br from-[#2a1a4a] to-[#1a2a4a] border-2 border-white/20 text-[1.4rem] cursor-pointer flex items-center justify-center transition-transform duration-150 hover:scale-[1.08] hover:border-white/40"
              onClick={() => setShowAvatars((v) => !v)} title="Cambiar avatar">{avatarEmoji}</button>
            <div>
              <div className="text-sm font-extrabold text-white leading-tight">{profile?.display_name ?? "Jugador"}</div>
              <div className="text-[0.68rem] text-white/40 font-bold">{completedCount}/{totalLevels} niveles · ⭐ {totalStars}</div>
            </div>
          </div>
          <div className="flex items-center gap-2">
            ...
            <div className="relative group flex items-center gap-1 bg-white/[0.07] border border-white/10 rounded-full px-2.5 py-1 text-xs font-extrabold text-white cursor-pointer hover:bg-white/12 transition-colors"
              onClick={() => router.push("/tienda")}>
              🪙<span className="text-[#FFD700]">{localCoins}</span>
              <span className="absolute -bottom-7 left-1/2 -translate-x-1/2 bg-[#1a1a25] border border-white/10 text-white/70 text-[0.65rem] font-bold px-2 py-0.5 rounded-lg whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity duration-150 pointer-events-none">Tienda 🛒</span>
            </div>
            <LivesPill lives={localLives} livesResetAt={localLivesResetAt} userId={user?.id}
              onRestored={(newLives, nextRegen) => { setLocalLives(newLives); setLocalLivesResetAt(nextRegen); }} />
            <button className="w-8 h-8 rounded-full bg-white/8 border border-white/12 text-white/70 text-lg cursor-pointer flex items-center justify-center transition-colors duration-150 hover:bg-white/15 tracking-[-1px]"
              onClick={() => setShowMenu((v) => !v)} aria-label="Menú">⋯</button>
          </div>
        </header>
        ...
        {showMenu && (
          <div className="fixed top-16 right-[max(16px,calc(50vw-244px))] z-9999 bg-[#1a1a25] border border-white/12 rounded-2xl p-1.5 min-w-47.5 shadow-[0_8px_32px_rgba(0,0,0,0.5)] animate-[fadeIn_0.15s_ease]">
            <button className="w-full px-3.5 py-2.5 rounded-xl border-none bg-transparent text-white/85 font-[Nunito,sans-serif] text-sm font-bold cursor-pointer text-left flex items-center gap-2 transition-colors duration-150 hover:bg-white/8"
              onClick={() => { setShowAvatars(true); setShowMenu(false); }}>🎭 Cambiar avatar</button>
            <div className="h-px bg-white/8 my-1" />
            <button className="w-full px-3.5 py-2.5 rounded-xl border-none bg-transparent text-white/85 font-[Nunito,sans-serif] text-sm font-bold cursor-pointer text-left flex items-center gap-2 transition-colors duration-150 hover:bg-white/8"
              onClick={() => { setShowMenu(false); router.push("/etapa"); }}>🌍 Cambiar etapa</button>
            <div className="h-px bg-white/8 my-1" />
            <button className="w-full px-3.5 py-2.5 rounded-xl border-none bg-transparent text-white/85 font-[Nunito,sans-serif] text-sm font-bold cursor-pointer text-left flex items-center gap-2 transition-colors duration-150 hover:bg-white/8"
              onClick={() => { setShowMenu(false); router.push("/tienda"); }}>🛒 Tienda</button>
            <div className="h-px bg-white/8 my-1" />
            ...
            <button className="w-full px-3.5 py-2.5 rounded-xl border-none bg-transparent text-[#ef9a9a] font-[Nunito,sans-serif] text-sm font-bold cursor-pointer text-left flex items-center gap-2 transition-colors duration-150 hover:bg-[rgba(239,83,80,0.12)]"
              onClick={async () => { await signOut(); router.replace("/auth"); }}>🚪 Cerrar sesión</button>
          </div>
        )}
          ...
          <div className="fixed inset-0 z-300 bg-black/75 backdrop-blur-[10px] flex items-center justify-center animate-[fadeIn_0.2s_ease]"
            ...
            <div className="bg-[#1a1a25] border border-white/12 rounded-3xl p-7 flex flex-col items-center gap-5 w-[min(340px,90vw)]"
              ...
              <div className="grid grid-cols-3 gap-3 w-full">
                {AVATARS.map((av, i) => (
                  <button key={i}
                    className={`aspect-square rounded-2xl border-2 text-4xl cursor-pointer flex items-center justify-center transition-all duration-150 disabled:opacity-50 disabled:cursor-wait hover:bg-white/12 ${profile?.avatar_id === i + 1 ? "border-[#FFD700] bg-[rgba(255,215,0,0.12)] shadow-[0_0_16px_rgba(255,215,0,0.3)]" : "border-white/10 bg-white/6"}`}
                    onClick={() => handleAvatarSelect(i)} disabled={savingAvatar}>{av}</button>
                ))}
              </div>
              <button className="w-full py-3 rounded-xl border border-white/12 bg-white/6 text-white/60 font-[Nunito,sans-serif] text-sm font-bold cursor-pointer transition-colors duration-150 hover:bg-white/10"
                onClick={() => setShowAvatars(false)}>Cancelar</button>
            </div>
          </div>
        )}
            ...
            return (
              <div key={island.id} className="w-full">
                ...
                <div className="flex items-center gap-3 px-5 py-3.5 mx-3 mb-1 rounded-2xl border border-transparent"
                  ...
                  <div>
                    <div className="font-[FredokaOne,sans-serif] text-[0.95rem] leading-tight" style={{ color: theme.label }}>{island.name}</div>
                    <div className="text-[0.68rem] font-bold opacity-55 mt-px" style={{ color: theme.label }}>{completed}/{islandLevels.length} completados</div>
                  </div>
                </div>
                ...
                <div className="relative w-full" style={{ height: blockH }}>
                  ...
                  {islandLevels.map((level, i) => {
                    ...
                    return (
                      <div key={level.id} className="absolute -translate-x-1/2 -translate-y-1/2 flex flex-col items-center z-2"
