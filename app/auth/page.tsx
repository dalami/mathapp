  ...
  useEffect(() => {
    ...
    supabase.auth
      ...
      .then(({ error }) => {
        ...
        // Si no hay error, onAuthStateChange dispara, user se setea
        // y tu useEffect existente redirige a /etapa solo.
      });
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);
    ...
    <div className="bg-white/6 backdrop-blur-xl border border-white/12 rounded-[28px] p-10 w-full max-w-105 relative z-10 shadow-[0_32px_64px_rgba(0,0,0,0.4)]">
      ...
      <form onSubmit={handleSubmit} noValidate>
        ...
        {mode === "register" && (
          <div className="mb-3.5">
            ...
            <input
              ...
              className="w-full px-4 py-3.5 bg-white/8 border border-white/12 rounded-xl text-white text-[0.95rem] outline-none transition-all duration-200 placeholder:text-white/30 focus:border-[#FFD700] focus:bg-white/12"
            />
          </div>
        )}
        ...
        <button
          ...
          className="w-full py-3.5 mt-3 bg-white text-[#1a1a4e] border-none rounded-2xl text-[0.95rem] font-bold cursor-pointer transition-all duration-200 flex items-center justify-center gap-3 shadow-[0_4px_12px_rgba(0,0,0,0.2)] hover:-translate-y-0.5 hover:shadow-[0_8px_20px_rgba(0,0,0,0.25)] active:translate-y-0"
        >
